...
export const login = (credential) => {//credentail is client input user-name and password 谁调用，参数谁提供
...
export const register = (data) => {//data is client input registration info
...
export const getTopGames = () => {
    return fetch(topGamesUrl).then((response) => {//默认是method: 'GET'
        if (response.status !== 200) {
...
const getGameDetails = (gameName) => {//调用者提供gameName
...
export const searchGameById = (gameId) => {//调用者提供gameId，返回videos[], streams[], clips[]
...
export const searchGameByName = (gameName) => {
    return getGameDetails(gameName).then((data) => {//data由getGameDetails(gameName)返回，得到gameId
        if (data && data.id) {
...
const favoriteItemUrl = `${SERVER_ORIGIN}/favorite`;//add,delete,get共用一个url
